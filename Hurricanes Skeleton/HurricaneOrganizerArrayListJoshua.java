 ...
 */
public class HurricaneOrganizerArrayListJoshua
{
    private ArrayList<Hurricane> hurricanes;

    /**
     * Constructor for class HurricaneOrganizerArrayListJoshua.
     * 
     * @throws IOException  if file with the hurricane information cannot be found
     * 
     * @param   filename    the name of the file
     */
    public HurricaneOrganizerArrayListJoshua(String filename)throws IOException
    {
        readFile(filename);   
    }
    ...
    public int findMaxPressure( )
    {
        ...
        for(int i=0;i<hurricanes.size();i++)
        {
            maxPressure=Math.max(maxPressure,hurricanes.get(i).getPressure());
        }
        return maxPressure;
    }
    ...
    public int findMinPressure( )
    {
        ...
        for(int i=0;i<hurricanes.size();i++)
        {
            minPressure=Math.min(minPressure,hurricanes.get(i).getPressure());
        }
        return minPressure;
    }
    ...
    public double calculateAverageWindSpeed( )
    {
        ...
        for(int i=0;i<hurricanes.size();i++)
        {
            speedSum+=hurricanes.get(i).getSpeed();
        }
        return speedSum/hurricanes.size();
    }
    ...
    public double calculateAveragePressure( )
    {
        ...
        for(int i=0;i<hurricanes.size();i++)
        {
            pressureSum+=hurricanes.get(i).getPressure();
        }
        return pressureSum/hurricanes.size();
    }
    ...
    public double calculateAverageCategory( )
    {
        ...
        for(int i=0;i<hurricanes.size();i++)
        {
            categorySum+=hurricanes.get(i).getCategory();
        }
        return categorySum/hurricanes.size();
    }
    ...
    public void sortYears()
    {
        ...
        {
            int mindex=outer;
            for(int inner=outer;inner<hurricanes.size();inner++)
            {
    ...
    public void sortNames()
    {
        ...
        for(int outer=1;outer<hurricanes.size();outer++)
        {
            ...
            while(maxdex>=0&&hurricanes.get(maxdex).compareNameTo(temp)>0)
            {
                hurricanes.set(maxdex+1,hurricanes.get(maxdex));
                maxdex--;
            }
            hurricanes.set(maxdex+1,temp);
        }
    }
    ...
    private void sortPressuresHelper (int start, int end)
    {
        ...
        for(int outer=start;outer<end;outer++)
        {
            ...
            {
                if(hurricanes.get(inner).comparePressureTo(hurricanes.get(maxdex))>0)
                {
                    maxdex=inner;
                }
            }
    ...
    public ArrayList<Hurricane> searchYear(int year)
    {
        ...
        {
            if(hurricanes.get(index).getYear()==year)
            {
                counter++;
            }
        }
     ...
     * @return a Hurricane ArrayList of all objects in hurricanes with specified name. 
     ...
     * @return  a Hurricane ArrayList of all Hurricane objects with a specified name. 
     *          Returns null if there are no matches
     */
     ...
     * @return  a Hurricane ArrayList with objects from hurricanes with specified name. 
     *          Returns null if there are no matches
     */
    private ArrayList<Hurricane> retrieveMatchedNames (String name, int index)
    {
        ...
        {
            matches.add(hurricanes.get(findex+i));
        }
        return matches;
    }
     ...
     * 
     * @param   hurs    the Hurricane ArrayList to be printed
     */
    ...
    {
        HurricaneOrganizerArrayListJoshua cane = 
            new HurricaneOrganizerArrayListJoshua("hurricanedata.txt");
